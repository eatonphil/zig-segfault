...
fn expect_json_file_equals(
    ...
    inline for (@typeInfo(T).Struct.fields) |field| {
        if (comptime std.mem.eql(u8, field.name, "timestamp")) {
            ...
            try std.testing.expectEqualSlices(u8, @field(expected, field.name), @field(parsed, field.name));
        }
    }
}
...
pub fn main() !void {
    ...
    _ = args.next(arena.allocator());
    while (args.next(arena.allocator())) |arg_or_err| {
